...
from GroupMusic.config import ARQ_API_KEY
from GroupMusic.config import BOT_NAME as bn
from GroupMusic.config import DURATION_LIMIT
from GroupMusic.config import UPDATES_CHANNEL as updateschannel
from GroupMusic.config import que
from GroupMusic.function.admins import admins as a
from GroupMusic.helpers.admins import get_administrators
from GroupMusic.helpers.channelmusic import get_chat_id
from GroupMusic.helpers.errors import DurationLimitError
from GroupMusic.helpers.decorators import errors
from GroupMusic.helpers.decorators import authorized_users_only
from GroupMusic.helpers.filters import command, other_filters
from GroupMusic.helpers.gets import get_file_name
from GroupMusic.services.callsmusic import callsmusic, queues
...
from GroupMusic.services.downloaders import youtube
...
async def playlist(client, message):
    ...
    msg = "**Sedang dimainkan** dalam {}".format(message.chat.title)
    msg += "\n- " + now_playing
    msg += "\n- Req by " + by
    temp.pop(0)
    if temp:
...
async def p_cb(b, cb):
    ...
    if type_ == "playlist":
        ...
        now_playing = temp[0][0]
        by = temp[0][1].mention(style="md")
        msg = "**Sedang dimainkan** dalam {}".format(cb.message.chat.title)
        msg += "\n- " + now_playing
        msg += "\n- Req by " + by
        temp.pop(0)
        if temp:
...
async def play(_, message: Message):
        ...
        for administrator in administrators:
            if administrator == message.from_user.id:
                ...
                try:
                    ...
                    # print(e)
                    await lel.edit(
                        f"<b>🔴 Flood Wait Error 🔴 \nPengguna {user.first_name} tidak dapat bergabung dengan grup Anda karena berat permintaan untuk userbot! Pastikan pengguna tidak diblokir dalam group."
    ...
    try:
        ...
        await lel.edit(
            f"<i> {user.first_name} Userbot tidak ada dalam obrolan ini, Minta admin untuk mengirim / play perintah untuk pertama kalinya atau menambahkan {user.first_name} secara manual</i>"
        )
        return
    ...
    if audio:
        ...
        title = file_name
        thumb_name = "https://telegra.ph/file/3c7889bafcc86adafc697.jpg"
    ...
    if chat_id in callsmusic.pytgcalls.active_calls:
        ...
        await message.reply_photo(
            photo="final.png",
            reply_markup=keyboard,
            caption="▶️ **Dimainkan** di sini lagu tersebut diminta oleh {} via Youtube Music 😜".format(
        ...
        for administrator in administrators:
            if administrator == message_.from_user.id:
                ...
                try:
                    await USER.join_chat(invitelink)
                    await USER.send_message(
                        message_.chat.id, "Saya bergabung dengan grup ini untuk memainkan musik di VC"
                    )
                    await lel.edit(
                        "<b>helper userbot bergabung dengan obrolan Anda</b>",
                    )
                    ...
                    # print(e)
                    await lel.edit(
                        f"<b>🔴 Flood Wait Error 🔴 \nPengguna {user.first_name} tidak dapat bergabung dengan grup Anda karena berat permintaan untuk userbot! Pastikan pengguna tidak dibanned dalam grup."
                        "\n\nAtau secara manual tambahkan asisten ke Grup Anda dan coba lagi</b>",
                    )
    try:
        await USER.get_chat(chid)
        # lmoa = await client.get_chat_member(chid,wew)
