...
from GroupMusic.modules.play import arq
...
async def ytmusic(client, message: Message):
    ...
    if not urlissed:
        await pablo.edit("Invalid Perintah Syntax, Silakan Periksa Menu Bantuan Untuk Tahu Lebih Banyak!")
        return
    ...
    try:
        ...
        with youtube_dl.YoutubeDL(opts) as ytdl:
            ...
            if duration > DURATION_LIMIT:
                await pablo.edit(
                    f"❌ Video lebih lama dari {DURATION_LIMIT} menit tidak diperbolehkan, video yang disediakan adalah {duration} menit(s)"
                )
                is_downloading = False
                return
    ...
    await client.send_video(
        ...
        progress_args=(
            pablo,
            c_time,
            f"`Uploading {urlissed} Lagu dari Youtube Music!`",
            file_stark,
        ),
    )
    await pablo.delete()
